...
<div class="wrap">
    ...
    <h2>Videos Lightbox (Trash) <a href="admin.php?page=page_video_lightbox&parent=<?php echo $_GET['parent']; ?>" class="add-new-h2">Back</a></h2>
    ...
    class GalleriesMainPageTable extends WP_List_Table{
        ...
        public function column_post_title( $item ){
            ...
            $actions = array(
                'edit' => sprintf( '<a href="?page=%s&action=%s&ID=%s&parent=%s">Restore</a>', 'page_video_lightbox_delete', 'untrash' , $item['ID'], $item['post_parent'] ),
                'delete' => sprintf( '<a href="?page=%s&action=%s&ID=%s&parent=%s">Delete</a>', 'page_video_lightbox_delete', 'delete', $item['ID'], $item['post_parent'] )
                );
        ...
        public function column_post_status( $item ){
            ...
            switch($item['post_status']){
                case 'publish':
                    $status = 'Visible';
                    $actions = array(
                        'delete' => sprintf( '<a href="?page=%s&action=%s&ID=%s">Hide</a>', 'page_video_lightbox_edit', 'hide', $item['ID'] )
                        );
                    break;
                case 'draft':
                    $status = 'Not Visible';
                    $actions = array(
                        'edit' => sprintf( '<a href="?page=%s&action=%s&ID=%s">Show</a>', 'page_video_lightbox_edit', 'show' , $item['ID'] ),
                        );
                    break;
                case 'trash':
                    $status = 'Deleted';
